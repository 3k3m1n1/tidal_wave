...
/*
  TO-DO: 
  [*] start with attractor code
  [*] hide GUI on startup
  
  [*] change particle colors to ocean blues
  [...] tweak particle parameters to feel like waterbending / ultimate control
        (v strong attraction, slightly-less-strong cohesion, low cohesion size to avoid angry bees look, display size > collider size for overlap, high attractor mass for better range)
  [ ] change particle sprite/appearance
  [ ] reduce bloom (use misc > bloom_demo for reference)
  
  [*] swap left/right click (so left to drag attractors, right to spawn particles]
  [ ] attach attractor #1 to mouse cursor
  
  [ ] create keyboard toggle for using attractors or normal gravity
  [ ] create timer for idle activity: after x seconds of no/barely any movement, turn gravity on
  [ ] new movement should reset toggle + timer
  
  [ ] connect kinect + import/setup library
  ...
  [ ] ??interactive sound??
        (start off realistic, then remix / have fun w it - samplefocus is your friend)
  
  [ ] FX: better liquid shader? gaussian blur?
  
  [ ] fullscreen spanning multiple displays, or twin windows (applets)?
